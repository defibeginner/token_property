...
class TransferCostDetector(object):
    ...
    def __init__(self, _config_file: str, day: str):
        ...
        w3_sync = Web3(Web3.HTTPProvider(config['RpcUrl']))
        self.pool_contract = w3_sync.eth.contract(address=pool_address, abi=self._erc20_abi)
    ...
    async def search_transfer_1trans(self, trx, block):
        try:
            ...
            trx_receipt = await self._w3.eth.getTransactionReceipt(txid)
            print('2' * 20)
            print(trx_receipt)
            ...
            await asyncio.sleep(1)
            logs = await self.pool_contract.events.Transfer().processReceipt(trx_receipt, EventLogErrorFlags.Warn)
            logs_summary = list()
            for log in logs:
    ...
    async def search_transfer(self):
        ...
        end_block_id = 16308190

        for block_id in range(start_block_id, end_block_id + 1):
            block = await self._w3.eth.get_block(block_id, True)
            if block is None:
                continue
            ...
            for i, trx in enumerate(block['transactions']):
                t = threading.Thread(target=asyncio.run, args=(self.search_transfer_1trans(trx, block),))
                t.start()
