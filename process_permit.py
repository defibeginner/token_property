...
def process_logs():
    ...
    filename = 'results/permit/permit_v2.log'
    df = pd.DataFrame(columns=['symbol', 'address', 'impl_contract', 'permit_in_abi', 'permit_func', 'signature',
                               ...
                               'error', 'is_permit_from_our_result', 'domain_separator', 'permit_type'])
    with open(filename) as f:
        for line in f:
            line = line[:-1]
            line = line[line.find('{'):]
    ...
    df.index.name = 'id'
    df.to_csv('results/permit/permit_9_crosscheck.csv', index=True)
